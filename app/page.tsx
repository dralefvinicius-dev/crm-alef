    ...
    <>
      ...
      <div className="layout">
        ...
        <div className="main">
          ...
          {/* Header desktop */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }} className="desktop-header">
            <div>
            ...
            <div>
              <div className="stats">
                {[
                  ...
                  { label: 'Consultas', val: consultas, cor: '#059669' },
                  { label: 'Contratos', val: contratos, sub: `${taxa}%`, cor: GOLD },
                  { label: 'Perdidos', val: perdidos, cor: '#dc2626' },
                ].map(s => (
                  <div key={s.label} style={{ background: '#fff', borderRadius: 12, padding: '16px', borderLeft: `4px solid ${s.cor}`, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                    ...
                    <div style={{ fontSize: 30, fontWeight: 700, color: s.cor }}>{s.val}</div>
                    {s.sub && <div style={{ fontSize: 11, color: '#9ca3af' }}>Taxa: {s.sub}</div>}
                  </div>
                ))}
              </div>
              {leadsFrios.length > 0 && (
                <div style={{ background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 10, padding: '12px 16px', fontSize: 13, color: '#92400e', marginBottom: 16 }}>
          ...
          {!loading && aba === 'leads' && (
            <div>
              ...
              <div className="leads-table-wrap">
                <div style={{ background: '#fff', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', minWidth: 500 }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr style={{ borderBottom: '2px solid #f3f4f6' }}>
                        {['Lead', 'Assunto', 'Fase', 'Ações'].map(h => (
                    ...
                    <tbody>
                      {leadsFiltrados.length === 0 && <tr><td colSpan={4} style={{ textAlign: 'center', padding: 40, color: '#9ca3af' }}>Nenhum lead encontrado.</td></tr>}
                          ...
                          <tr key={l.id} style={{ borderBottom: '1px solid #f3f4f6' }}
                            ...
                            <td style={{ padding: '12px 14px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <div style={{ width: 4, height: 36, borderRadius: 4, background: TEMP_COR[l.temp || ''] || '#e5e7eb', flexShrink: 0 }} />
                            ...
                            <td style={{ padding: '12px 14px', color: '#6b7280', maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.assunto}</td>
                            <td style={{ padding: '12px 14px' }}>
                              <span style={{ padding: '3px 8px', borderRadius: 20, fontSize: 11, fontWeight: 600, background: fc.bg, color: fc.color, whiteSpace: 'nowrap' }}>{l.fase}</span>
                            </td>
                            <td style={{ padding: '12px 14px' }}>
                              <div style={{ display: 'flex', gap: 10 }}>
                                <button onClick={() => abrirEditar(l)} style={{ fontSize: 12, color: '#2563eb', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Editar</button>
                                <button onClick={() => excluirLead(l.id!, l.nome)} style={{ fontSize: 12, color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Excluir</button>
                              </div>
                            </td>
                          </tr>
                        )
          ...
          {!loading && aba === 'funil' && (
            <div className="funil-grid">
              {FASES.map(fase => {
                ...
                return (
                  <div key={fase} style={{ background: '#fff', borderRadius: 12, padding: 12, minHeight: 100, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', borderTop: `3px solid ${fc.color}` }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: fc.color, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.4 }}>{fase} ({grupo.length})</div>
                    {grupo.map(l => (
                      <div key={l.id} onClick={() => abrirEditar(l)}
                        style={{ background: '#f8f9fa', borderRadius: 8, padding: '8px 10px', marginBottom: 6, cursor: 'pointer', borderLeft: `3px solid ${TEMP_COR[l.temp || ''] || '#e5e7eb'}` }}
          ...
          {!loading && aba === 'historico' && (
            <div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
                <select style={{ ...inp, width: 'auto', flex: 1 }}>
              ...
              <div style={{ background: '#fff', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                ...
                {historico.map((h, i) => (
                  <div key={h.id} style={{ padding: '14px 18px', borderBottom: i < historico.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                    ...
                    <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 4 }}>{h.data}{h.resultado ? ` · ${h.resultado}` : ''}</div>
      ...
      {modalHist && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', padding: '24px 20px', width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button onClick={() => setModalHist(false)} style={{ flex: 1, padding: '12px', fontSize: 14, border: '1px solid #e5e7eb', borderRadius: 10, background: '#fff', cursor: 'pointer' }}>Cancelar</button>
