...
export default function Home() {
  ...
  const carregar = useCallback(async () => {
    ...
    if (e1) setErro('Erro ao carregar leads: ' + e1.message)
    if (e2) setErro('Erro ao carregar histórico: ' + e2.message)
    setLeads(l || [])
    setHistorico(h || [])
    setLoading(false)
  }, [])
  ...
  const salvarLead = async () => {
    ...
    setSaving(false)
    if (errSave) { alert('Erro ao salvar: ' + errSave.message); return }
    setModalLead(false)
    carregar()
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={abrirNovo} className="bg-gray-900 text-white px-4 py-2 rounded-lg text-sm hover:bg-gray-700 transition">+ Novo lead</button>
      </div>

      {erro && <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700 mb-4">{erro}</div>}
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              <h3 className="text-sm font-medium mb-3">Por origem</h3>{barChart('origem')}
            </div>
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              <h3 className="text-sm font-medium mb-3">Por área jurídica</h3>{barChart('area')}
            </div>
          </div>
        </div>
      )}
      ...
      {!loading && aba === 'leads' && (
        <div>
          <div className="flex gap-2 mb-4 flex-wrap">
            <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar por nome, cidade ou assunto..."
            ...
            <select value={filtroFase} onChange={e => setFiltroFase(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm">
              <option value="">Todas as fases</option>{FASES.map(f => <option key={f}>{f}</option>)}
            </select>
            <select value={filtroTemp} onChange={e => setFiltroTemp(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b text-xs text-gray-500 font-medium">
                  <th className="text-left px-4 py-3">Nome</th><th className="text-left px-4 py-3">Assunto</th>
                  <th className="text-left px-4 py-3">Fase</th><th className="text-left px-4 py-3">Temp.</th>
                  <th className="text-left px-4 py-3">Origem</th><th className="text-left px-4 py-3">Próx. ação</th><th className="px-4 py-3"></th>
                </tr>
              </thead>
              <tbody>
                {leadsFiltrados.length === 0 && <tr><td colSpan={7} className="text-center py-12 text-gray-400">Nenhum lead encontrado.</td></tr>}
                {leadsFiltrados.map(l => (
                  <tr key={l.id} className="border-b last:border-0 hover:bg-gray-50 transition">
                    <td className="px-4 py-3 font-medium">
                      {l.wa && <a href={`https://wa.me/${l.wa.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="text-green-600 mr-1 hover:text-green-800">↗</a>}
                      {l.nome}
                    </td>
                    <td className="px-4 py-3 text-gray-600 max-w-32 truncate">{l.assunto}</td>
                    <td className="px-4 py-3"><span className={`badge ${FASE_CORES[l.fase||''] || 'bg-gray-100 text-gray-600'}`}>{l.fase}</span></td>
                    <td className="px-4 py-3"><span className={`badge ${TEMP_CORES[l.temp||''] || 'bg-gray-100 text-gray-600'}`}>{l.temp}</span></td>
          ...
          {FASES.map(fase => {
            ...
            return (
              <div key={fase} className="bg-white rounded-xl border border-gray-100 p-3 min-h-32">
                ...
                {grupo.map(l => (
                  <div key={l.id} onClick={() => abrirEditar(l)} className="bg-gray-50 rounded-lg p-2 mb-2 cursor-pointer hover:bg-blue-50 transition border border-gray-100">
        ...
        <div>
          <div className="flex gap-2 mb-4">
            ...
            <button onClick={() => { setHistForm({ data: new Date().toISOString().slice(0,10) }); setModalHist(true) }}
              className="border border-gray-200 rounded-lg px-4 py-2 text-sm hover:bg-gray-50">+ Registrar atendimento</button>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 divide-y">
            {historico.length === 0 && <div className="text-center py-12 text-gray-400 text-sm">Nenhum atendimento registrado.</div>}
            {historico.map(h => (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={salvarLead} disabled={saving} className="px-4 py-2 text-sm bg-gray-900 text-white rounded-lg hover:bg-gray-700 disabled:opacity-50">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input value={histForm.resultado || ''} onChange={e => setHistForm(p => ({ ...p, resultado: e.target.value }))}
                  placeholder="Ex: Agendou consulta" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
              </div>
              <div className="col-span-2">
